...
import { getQueryParams, sendOk, wrapHandler } from '../middleware'
import { resolveCompanyKey } from '../helpers'
import { requireAuth } from '../auth-middleware'
import type { Router } from '../router'

export function register(router: Router): void {
  router.get('/api/companies', wrapHandler(requireAuth(async (_req, res) => {
    const pool = getPool()
    ...
    sendOk(res, { rows: (rows as Record<string, unknown>[]).map(r => ({ companyCode: String(r.companyCode ?? ''), companyName: String(r.companyName ?? '') })) })
  })))

  router.get('/api/warehouses', wrapHandler(requireAuth(async (req, res) => {
    ...
    sendOk(res, { rows: (rows as Record<string, unknown>[]).map(r => ({ code: String(r.code ?? ''), description: String(r.description ?? ''), isDefault: Boolean(r.isDefault) })) })
  })))

  router.get('/api/vendors', wrapHandler(requireAuth(async (req, res) => {
    const company = (getQueryParams(req.url).company || '').trim()
    if (!company) return sendOk(res, { rows: [] })
    const pool = getPool()
    const key  = await resolveCompanyKey(pool, company)
    ...
    const rows = await stmt.all(key, key)
    sendOk(res, { rows: (rows as Record<string, unknown>[]).map(r => ({ code: String(r.code ?? ''), description: String(r.description ?? '') })) })
  })))

  router.get('/api/channel-templates', wrapHandler(requireAuth(async (req, res) => {
    const company = (getQueryParams(req.url).company || '').trim()
    if (!company) return sendOk(res, { rows: [] })
    const pool = getPool()
    const key  = await resolveCompanyKey(pool, company)
